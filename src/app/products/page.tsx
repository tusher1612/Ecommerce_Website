...
const ProductsList = async ({ searchParams }: { searchParams?: { q?: string } }) => {
  ...
  const products = query ? await querySearch(query) : await fetchProducts();
  const isSearchActive = Boolean(query);

  return (
    <div className="min-h-screen p-6 flex flex-col">
      <h1 className="text-2xl font-bold mb-4">{isSearchActive ? `Search Results for "${query}"` : "All Products"}</h1>
