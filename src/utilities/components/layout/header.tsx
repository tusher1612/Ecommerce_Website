...
import { Heart, LogOutIcon, Search, ShoppingCart, User } from "lucide-react";
...
const Header = () => {
  ...
  return (
    <header className="bg-walmart-default flex md:flex-row flex-col items-center justify-between px-10 py-7 space-x-5 gap-5">
      ...
      <form
        ...
        className="flex items-center bg-white w-full md:w-2/4 rounded-full md:gap-x-7"
      >
        <input
          name="searchInput"
          type="text"
          ...
          className="flex-1 bg-white outline-none placeholder:text-sm ml-2 focus:bg-white focus:outline-none focus:ring-0"
        />
        <button
          type="submit"
          className="bg-yellow-300 rounded-full px-2 w-10 h-10 cursor-pointer"
        >
          <Search />
        </button>
      </form>
      ...
      <div className="flex space-x-3 pl-10">
        ...
        <div className="flex font-bold items-center text-white text-sm space-x-1.5 md:mt-0 mt-5 cursor-pointer">
          ...
          <span className="bg-yellow-300 rounded-full h-4 w-4 mb-6 ml-16 absolute text-center">
            {session ? sessionCart.length : cart.length}
          </span>
        </div>

        <div className="flex font-bold items-center text-white text-sm space-x-1.5 md:mt-0 mt-5">
          {!session ? (
            <Link href="/sign-in" className="ml-1 cursor-pointer">
              <User size={20} />
            </Link>
          ) : (
            <LogOutIcon
